...
describe("hiro-kit::cursor - buffers", () => {
  ...
  it.prop([buff_n(8192), fc.nat(2048), fc.nat(2048)])(
    "slice",
    (bytes, pos, size) => {
      let cursor = Cl.tuple({
  ...
  it.prop([bytesToRead(), bytesToGenerate(8192)])(
    "read-buff-8192-max and slice",
    (toRead, toGenerate) => {
      var data = new Uint8Array();
  ...
  it.prop([bytesToRead(), bytesToRead(), bytesToGenerate(8192)])(
    "read-buff-8192-max with limit and slice",
    (toRead, toIsolate, toGenerate) => {
      var data = new Uint8Array();
      for (let n of toGenerate) {
        data = concatTypedArrays(data, uint8toBytes(n));
      }
...
describe("hiro-kit::cursor - unsigned integers", () => {
  ...
  let arrayOfUint64 = fc.array(fc.bigUintN(64), {
    minLength: 0,
    maxLength: 1024,
  });
  ...
  let arrayOfUint128 = fc.array(fc.bigUintN(128), {
    minLength: 0,
    maxLength: 512,
  });
...
describe("hiro-kit::cursor - signed integers", () => {
  ...
  let arrayOfInt16 = fc.array(fc.bigIntN(16), {
    minLength: 0,
    maxLength: 1023,
  });
  ...
  let arrayOfInt32 = fc.array(fc.bigIntN(32), {
    minLength: 0,
    maxLength: 1023,
  });
  ...
  let arrayOfInt64 = fc.array(fc.bigIntN(64), {
    minLength: 0,
    maxLength: 1023,
  });
  ...
  let arrayOfInt128 = fc.array(fc.bigIntN(128), {
    minLength: 0,
    maxLength: 1023,
  });
