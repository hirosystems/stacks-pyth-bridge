...
use sha3::Keccak256;
use sha3::Digest;
use libsecp256k1::{Message, recover, Signature, RecoveryId, PublicKey};
...
pub async fn ping_bridge_service(config: &Config, ctx: &Context) -> Result<(), String> {
    ...
    for (guardian_id, public_key) in public_keys_epoch_1.iter() {
        ...
        let eth_address = {
            let public_key_hash: [u8; 32] = Keccak256::digest(&public_key_bytes).try_into().unwrap();
            public_key_hash[12..32].to_vec()
        };
        info!(ctx.expect_logger(), "{}:\t0x{} ({})", guardian_id, hex::encode(&eth_address), hex::encode(&public_key_bytes));
    }
    ...
    for (guardian_id, public_key) in public_keys_epoch_3.iter() {
        ...
        info!(ctx.expect_logger(), "{}:\t0x{}", guardian_id, hex::encode(&eth_address));
    }
...
pub async fn collect_guardians_public_keys(config: &Config, ctx: &Context) {
    info!(ctx.expect_logger(), "Collecting VAAs and computing guardians public keys");
            ...
            for (guardian_id, public_key) in new_public_keys.into_iter() {
                match public_keys.get(&guardian_id) {
                    None => {
                        ...
                        info!(ctx.expect_logger(), "Found new public key: {}\t{}", guardian_id, hex::encode(&key));
...
pub fn compute_secp256k1_recovery_from_vaa(vaa: &Vec<u8>, ctx: &Context) -> Result<Vec<(u8, PublicKey)>, String> {
    ...
    for i in 0..sig_len {
        ...
        debug!(ctx.expect_logger(), "{}:\t{}", guardian_id, hex::encode(&signature));
    ...
    for (guardian_id, signature_bytes, recovery_id_byte) in sigs.into_iter() {
        ...
        let public_key = recover(&message, &signature, &recovery_id).expect("unable to recover public key");
